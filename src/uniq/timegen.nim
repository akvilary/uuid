## Per-thread timestamp and clock sequence generation for UUID v1 and v6.
...
var
  lastTimestamp {.threadvar.}: uint64
  clockSeq {.threadvar.}: uint16
  nodeId {.threadvar.}: array[6, byte]
  initialized {.threadvar.}: bool
...
proc getTimestampAndClockSeq*(): tuple[ts: uint64, clockSeq: uint16, node: array[6, byte]] =
  ...
  ## Per-thread state with clock regression handling.
  if not initialized:
    initState()
    initialized = true
  let now = getTime()
